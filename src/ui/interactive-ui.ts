...
import { DuplicateGroup, DuplicateAction, DuplicateType } from '../types/duplicate';
...
export class InteractiveUI {
  ...
  private async showMainMenu(): Promise<void> {
    ...
    const summary = [
      ...
      chalk.green(`• Potential savings: ${formatFileSize(stats.potentialSavings)}`)
    ].join('\n');
  ...
  private async showDuplicateGroup(index: number): Promise<void> {
    ...
    console.log(chalk.green(`Potential savings: ${formatFileSize(group.potentialSavings)}`));
    console.log();
  ...
  private async selectIndividualActions(group: DuplicateGroup, groupIndex: number): Promise<void> {
    console.clear();
  ...
  private async deleteAllButOne(group: DuplicateGroup, groupIndex: number): Promise<void> {
    ...
    group.files.forEach((file, index) => {
      const action = index === keepIndex ? DuplicateAction.KEEP : DuplicateAction.DELETE;
      group.actions.set(file.path, action);
    });
  ...
  private async setGroupAction(group: DuplicateGroup, action: DuplicateAction): Promise<void> {
  ...
  private async generateReport(): Promise<void> {
    ...
    try {
      ...
      await reporter.generateReport(this.duplicateGroups, outputPath, this.baseDirectory);
