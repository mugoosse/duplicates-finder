...
export class UndoSystem {
  ...
  async recordOperation(operation: Omit<UndoOperation, 'id' | 'timestamp'>): Promise<string> {
    ...
    logger.info('Undo operation recorded', { operationId: undoOp.id, action: undoOp.action });
    return undoOp.id;
  }
  ...
  async undoOperation(operationId: string): Promise<boolean> {
    ...
    try {
      switch (operation.action) {
        ...
        default:
          logger.warn('Cannot undo operation type', { action: operation.action });
          return false;
      }
    ...
    for (const operation of this.operations) {
      if (operation.backupPath) {
        try {
          ...
          logger.warn('Failed to delete backup file', { backupPath: operation.backupPath, error });
        }
      }
    }
  ...
  private async loadHistory(): Promise<void> {
    try {
      ...
      logger.info('Undo history loaded', { operationCount: this.operations.length });
  ...
  private async saveHistory(): Promise<void> {
    try {
      ...
      await fs.writeFile(this.undoHistoryPath, JSON.stringify(this.operations, null, 2), 'utf-8');
