...
import { DuplicateGroup, DuplicateStats, DuplicateType } from '../types/duplicate';
import { formatFileSize, formatRelativeTime } from '../utils/file-utils';
import { logger } from '../utils/logger';

export class MarkdownReporter {
  
  async generateReport(duplicateGroups: DuplicateGroup[], outputPath: string, baseDirectory: string): Promise<void> {
    logger.info('Generating markdown report', { outputPath, groupCount: duplicateGroups.length });
    
    const stats = this.calculateStats(duplicateGroups);
    const report = this.buildReportContent(duplicateGroups, stats, baseDirectory);
    ...
    logger.info('Markdown report generated and formatted successfully', { outputPath });
  }

  private buildReportContent(duplicateGroups: DuplicateGroup[], stats: DuplicateStats, baseDirectory: string): string {
    ...
    duplicateGroups.forEach((group, index) => {
      ...
      report.push(`**Potential Savings:** ${formatFileSize(group.potentialSavings)}`);
      report.push(``);
      ...
      group.files.forEach(file => {
        ...
        report.push(`| \`${relativePath}\` | ${size} | ${modified} | ${action} |`);
      });
    ...
    report.push(``);
    report.push(`For experienced users, here are some sample commands to get started:`);
    report.push(``);
    report.push(`\`\`\`bash`);
  ...
  private calculateSavingsByType(duplicateGroups: DuplicateGroup[], type: DuplicateType): number {
  ...
  private async formatWithPrettier(filePath: string): Promise<void> {
    return new Promise((resolve, reject) => {
      const prettierProcess = spawn('npx', ['prettier', '--write', '--parser', 'markdown', filePath], {
