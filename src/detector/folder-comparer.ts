...
export class FolderComparer {
  ...
  async buildFolderStructure(directoryPath: string, basePath: string): Promise<FolderStructure> {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          const subStructure = await this.buildFolderStructure(entryPath, basePath);
  ...
  private calculateStructureHash(files: Map<string, string>, subdirectories: Map<string, FolderStructure>): string {
    const hash = createHash('sha256');
  ...
  async getFolderSize(folderPath: string): Promise<number> {
    ...
    try {
      ...
      logger.error('Failed to calculate folder size', { path: folderPath, error });
    }
