...
export async function reportCommand(options: ReportOptions): Promise<void> {
  ...
  if (!options.input) {
    console.log(chalk.red('Error: Input file is required for report generation'));
    console.log(chalk.gray('Use the scan command first to generate results, then specify the input file.'));
    process.exit(1);
  }
