...
export async function scanCommand(directory: string, options: ScanOptions): Promise<void> {
  ...
  try {
    ...
    spinner.succeed(`Found ${scanResult.totalFiles} files (${formatFileSize(scanResult.totalSize)})`);
  } catch (error) {
  ...
  try {
    ...
    spinner.succeed(`Found ${stats.totalGroups} duplicate groups with ${stats.totalFiles} files`);
    ...
    console.log(chalk.white(`• Total duplicate size: ${formatFileSize(stats.totalSize)}`));
    console.log(chalk.green(`• Potential space savings: ${formatFileSize(stats.potentialSavings)}`));
    
    if (stats.groupsByType['name-based'] > 0) {
      console.log(chalk.cyan(`• Name-based duplicates: ${stats.groupsByType['name-based']} groups`));
    }
    if (stats.groupsByType['content-based'] > 0) {
      console.log(chalk.cyan(`• Content-based duplicates: ${stats.groupsByType['content-based']} groups`));
    }
    if (stats.groupsByType['folder-based'] > 0) {
      console.log(chalk.cyan(`• Folder-based duplicates: ${stats.groupsByType['folder-based']} groups`));
    }
